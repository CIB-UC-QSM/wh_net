...
class QSMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        chi = torch.load(self.chi_files[idx]).float() * torch.empty(1).uniform_(0.5, 3)
        ...
        scale = torch.empty(1).uniform_(0.01, 10).item()
        ...
        snr = torch.empty(1).uniform_(25, 100).item()
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(dataset, batch_size=1, shuffle=True, pin_memory=torch.cuda.is_available())
